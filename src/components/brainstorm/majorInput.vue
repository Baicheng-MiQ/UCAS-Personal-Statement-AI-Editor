<template>
    <div>
        <ul class="flex flex-row font-bold text-xl overflow-x-scroll">

            <li v-for="(item, index) in major" :key="index"
                class="flex flex-row mx-2 mb-3 px-1 py-1">
                <p class="whitespace-nowrap underline">{{item}}</p>
                <!-- round cross icon -->
                <div class="my-auto rounded-full bg-gray-100 ml-1 cursor-pointer"
            ...
            <li>
                <div class="flex flex-row content-center mx-2 mb-3 pr-2 ">
                    <input type="text" class="w-52 h-full m-0.5 px-2 py-1 font-bold bg-white rounded-full"
                        :class="{'hidden': !addingMajor}"
                        maxlength="100" v-model="newMajor" @keyup.enter="addMajor" @focusout="focusOutAddMajor"
                        placeholder="New Major">

                    <!-- this button toggle add major input block -->
                    <div class="h-8 my-auto rounded-full  ml-2 cursor-pointer "
...
export default {
    ...
    methods: {
        ...
        focusOutAddMajor() {
            if(this.newMajor){this.addMajor()};
        }
    }
}
</script>
